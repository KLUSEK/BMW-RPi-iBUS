...
def device_property_changed(property_name, value, path, interface, device_path):
    ...
    device = dbus.Interface(bus.get_object(SERVICE_NAME, device_path), "org.freedesktop.DBus.Properties")
...
if __name__ == '__main__':
    ...
    # Power on adapter
    adapter.Set(ADAPTER_INTERFACE, "Powered", dbus.Boolean(1))
    # Set name to display in available connections
    adapter.Set(ADAPTER_INTERFACE, "Alias", CONNECTION_NAME)
    # Set pairable on
    adapter.Set(ADAPTER_INTERFACE, "Pairable", dbus.Boolean(1))
    # Set discoverable on
    adapter.Set(ADAPTER_INTERFACE, "Discoverable", dbus.Boolean(1))
    ...
    bus.add_signal_receiver(device_property_changed, bus_name=SERVICE_NAME, signal_name="PropertiesChanged", path_keyword="device_path", interface_keyword="interface")

    # listen for signal of changing properties
    bus.add_signal_receiver(player_changed, bus_name=SERVICE_NAME, dbus_interface="org.freedesktop.DBus.Properties", signal_name="PropertiesChanged", path_keyword="path")
